...
var newBuffer = function (fill: any, encoding?: any) {
    return Buffer.from ? Buffer.from(fill, encoding) : new Buffer(fill, encoding)
};

base64url.unescape = function unescape(str: string) {
...
base64url.escape = function escape(str: string) {
    return str.replace(/\+/g, '*')
        .replace(/\//g, '-')
        .replace(/=/g, '_');
};

base64url.encode = function encode(str: string) {
    return this.escape(newBuffer(str).toString('base64'));
};

base64url.decode = function decode(str: string) {
...
class Api {
     ...
     * @private
     */
    _hmacsha256(identifier: string, currTime: any, expire: number, base64UserBuf: any) {
    ...
    _genUserbuf(account: string, dwAuthID: number, dwExpTime: number,
        dwPrivilegeMap: any, dwAccountType: any, roomstr: any) {
        ...
        let offset = 0;
        let userBuf: any = Buffer.alloc(length);
    ...
    genSig(userid: string, expire: number, userBuf: any) {
 ...
 */
    ...
    */
    ...
    genPrivateMapKey(userid: string, expire: number, roomid: number, privilegeMap: any) {
     ...
     *  - privilegeMap == 0010 1010 == 42: Indicates that the UserID has only the permissions to enter the room and receive audio/video data.
     */
    genPrivateMapKeyWithStringRoomID(userid: string, expire: number, roomstr: string, privilegeMap: any) {
